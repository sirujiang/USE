...
class ClipShifterTestTimeTuning_multitemplates(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14", n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, dset_name=None):
        super(ClipShifterTestTimeTuning_multitemplates, self).__init__()
        clip, _, _ = load(arch, device=device, download_root=DOWNLOAD_ROOT)
        self.arch = arch
        self.device = device
        self.image_encoder = clip.visual
    ...
    def get_text_features(self):
        clip, _, _ = load(self.arch, device=self.device, download_root=DOWNLOAD_ROOT)
        temps_features = []
        for template in UNIFIED_TEMPLATES:
            texts_str = [template.format(cls_name) for cls_name in self.classnames]
            tokenized_text = tokenize(texts_str).to(self.device)
            with torch.no_grad():
                t_feature = clip.encode_text(tokenized_text)
                temp_features = t_feature / t_feature.norm(dim=-1, keepdim=True)
            temps_features.append(temp_features)   
       
        text_features = torch.stack(temps_features)
...
def get_shift_multitemplates(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False, dset_name=None):

    model = ClipShifterTestTimeTuning_multitemplates(device, classnames, None, arch=clip_arch,
                            n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, dset_name=dset_name)

    return model



class ClipLoraTestTimeTuning_multitemplates(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14", n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, dset_name=None):
        super(ClipLoraTestTimeTuning_multitemplates, self).__init__()
        self.arch = arch
        clip, _, _ = load(arch, device=device, download_root=DOWNLOAD_ROOT)
        self.device = device
        self.image_encoder = clip.visual
...
def get_clip_lora_multitemplates(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False, dset_name=None):
    model = ClipLoraTestTimeTuning_multitemplates(device, classnames, None, arch=clip_arch, n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, dset_name=dset_name)
    return model

class ClipTestTimeTuning_multitemplates(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, dset_name=None):
        super(ClipTestTimeTuning_multitemplates, self).__init__()
        clip, _, _ = load(arch, device=device, download_root=DOWNLOAD_ROOT)
        self.arch = arch
        self.device = device
        self.image_encoder = clip.visual
        ...
        self.classnames = classnames
        self.dset_name = dset_name
        self.text_feature_init = self.get_text_features()
    ...
    def reset(self):
        """
        对于多模板版本，目前没有可学习的 prompt 参数，
        但为了兼容 STS 等需要调用 model.reset() 的方法，
        在这里将 text_feature_init 重新计算一遍，相当于“恢复初始状态”。
        """
        with torch.no_grad():
            self.text_feature_init = self.get_text_features()
    ...
    def get_text_features(self):
        """
        计算并返回 text features。
        为了加速，多模板 text features 会缓存在本地文件中，后续优先直接读取。
        """
        # 构造缓存路径：<repo_root>/text_feature_cache/text_feats_<arch>_<dset>.pt
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        cache_dir = os.path.join(repo_root, "text_feature_cache")
        os.makedirs(cache_dir, exist_ok=True)

        arch_tag = self.arch.replace("/", "-")
        dset_tag = self.dset_name if self.dset_name is not None else "unknown"
        cache_path = os.path.join(cache_dir, f"text_feats_{arch_tag}_{dset_tag}.pt")

        # 如果已有缓存，直接读取
        if os.path.exists(cache_path):
            # 先加载到 CPU，再迁移到当前设备，避免将 int 误传给 map_location
            cached = torch.load(cache_path, map_location="cpu")
            return cached.to(self.device)

        # 否则按原逻辑计算并保存
        clip_model, _, _ = load(self.arch, device=self.device, download_root=DOWNLOAD_ROOT)
        temps_features = []
        for template in UNIFIED_TEMPLATES:
            texts_str = [template.format(cls_name) for cls_name in self.classnames]
            tokenized_text = tokenize(texts_str).to(self.device)
            with torch.no_grad():
                t_feature = clip_model.encode_text(tokenized_text)
                temp_features = t_feature / t_feature.norm(dim=-1, keepdim=True)
            temps_features.append(temp_features)
    ...
    def inference(self, image):
        ...
        text_features = self.text_feature_init
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
...
def get_coop_multitemplates(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False, dset_name=None):

    model = ClipTestTimeTuning_multitemplates(device, classnames, None, arch=clip_arch,
                            n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, dset_name=dset_name)
